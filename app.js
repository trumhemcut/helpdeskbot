...
bot.dialog('SubmitTicket',
    [
        ...
        (session, result, next) => {
            if (!session.dialogData.serverity) {
                session.dialogData.serverity = result.response.entity
            }
        ...
        (session, result, next) => {
            ...
            var message = `Great! I'm going to create a ${session.dialogData.severity} severity ticket in the ${session.dialogData.category} category` +
                `The description: "${session.dialogData.description}". Can you please confirm?`
